...
> 두 테이블의 속성을 모두 포함하는 새로운 테이블을 만드는 방법.
...
- #### INNER JOIN
  ...
  A 테이블의 각각의 튜플에 대해, B 테이블에 칼럼 값이 같은 튜플이 있으면 두 튜플을 연결해 inner join에 포함되고, 그렇지 않으면 버려진다.
...
- #### LEFT OUTER JOIN
  ...
  A 테이블의 각각의 튜플에 대해, B 테이블에 칼럼 값이 같은 튜플이 있으면 두 튜플을 연결하고, 없으면 B 테이블의 속성 값을 null로 채운다.
