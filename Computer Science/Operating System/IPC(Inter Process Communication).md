...
프로세스는 커널이 제공하는 IPC 설비를 이용해 프로세스간 통신을 할 수 있게 한다.
...
1. ##### 익명 PIPE
   ...
   >
   >
   > 매우 간단하게 사용할 수 있는 장점이 있고, 단순한 데이터 흐름을 가질 땐 파이프를 사용하는 것이 효율적이다. 단점으로는 전이중 통신을 위해 2개를 만들어야 할 때는 구현이 복잡해진다.
...
2. ##### Named PIPE(FIFO)
   ...
   >
   > Named 파이프는 임의의 프로세스들 사이 통신에 사용한다.
   >
